...
<meta name="description" content="AI-powered travel guide for Japan">
<link rel="manifest" href="manifest.json">
<title>TABI — AI Travel Guide</title>
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@300;400;500;700&family=Noto+Sans+KR:wght@300;400;500&family=Noto+Sans+SC:wght@300;400;500&family=Playfair+Display:ital,wght@0,400;0,700;1,400&display=swap" rel="stylesheet">
<style>
:root {
  --bg: #0f0e0c;
  --surface: #1a1916;
...
.q-nav { display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; }
...
.result-body { flex:1; overflow-y:auto; padding:0 0 100px; }
...
<div id="screen-splash" class="screen active">
  <div class="splash-logo display">TABI</div>
...
<div id="screen-home" class="screen">
  <div class="q-header">
    <div class="q-nav">
      ...
      <button class="btn btn-ghost" onclick="showScreen('screen-premium')" style="font-size:18px;padding:4px 8px">👑</button>
    </div>
...
<div id="screen-questions" class="screen">
  <div class="q-header">
    <div class="q-nav">
      ...
      <span class="q-progress-text" id="q-progress-text">0 / 5</span>
    </div>
    <div class="q-progress-bar">
      <div class="q-progress-fill" id="q-progress-fill" style="width:0%"></div>
    </div>
    <div class="q-city display" id="q-city-name">京都</div>
    <div style="margin-top:6px"><span class="tag tag-teal" id="q-timing-badge">🗓 計画中</span></div>
...
<div id="screen-premium" class="screen">
  <div class="premium-header">
    ...
    <div class="tag tag-gold">PREMIUM</div>
    <div style="width:36px"></div>
  </div>

  <div class="premium-hero">
    <div class="premium-crown">👑</div>
    <div class="premium-title display" id="premium-title">TABIプレミアム</div>
...
{
  "destination_intro": {
    "summary": "目的地の簡潔な概要（2文程度）",
    "highlights": ["旅行者が知っておくべき特徴1", "特徴2", "特徴3"]
  },
...
tipsは3件。【店名・施設名のルール】有名観光地・世界遺産・有名神社仏閣など広く知られた施設は積極的に実名で記載してください。飲食店・宿泊施設は実在が確認できるものは実名で、不確かな場合は「地元の蕎麦店」「駅前の食堂」など一般的な表現にしてください。架空の固有名詞は絶対に使わないこと。短時間の場合は徒歩圏内・近距離でまわれるプランにしてください。スケジュールの最初の項目は最寄り駅や出発地点から始め、交通手段を明記してください。多日程の場合は各日の最初も同様に起点を示してください。`,
  ...
  en: {
    splashText: 'AI-powered travel guide for Japan\nRestaurants · Hotels · Sightseeing all in one',
...
{
  "destination_intro": {
    "summary": "Brief overview of the destination (2 sentences)",
    "highlights": ["Key fact travelers should know 1", "Key fact 2", "Key fact 3"]
  },
...
仅返回以下JSON格式，不要包含其他文字：
{
  "destination_intro": {
    "summary": "目的地简介（2句话）",
    "highlights": ["旅行者应该知道的特点1", "特点2", "特点3"]
  },
  "concept": "旅行概念（2-3句话）",
...
{
  "destination_intro": {
    "summary": "목적지 간략 소개 (2문장)",
    "highlights": ["여행자가 알아야 할 특징1", "특징2", "특징3"]
  },
...
function setLang(l) {
  ...
  document.querySelectorAll('.lang-btn').forEach(b => {
    b.classList.toggle('active',
      (l==='ja' && b.textContent.includes('日')) ||
      (l==='en' && (b.textContent.includes('EN') || b.textContent.includes('English'))) ||
      (l==='zh' && b.textContent.includes('中')) ||
      (l==='ko' && b.textContent.includes('한'))
    );
  });
  const t = T[lang];
  // splash
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('city-input').addEventListener('keydown', e => {
    if(e.key === 'Enter' && currentCity) goToQuestions();
  });
});
...
async function confirmCity() {
  ...
  try {
    const promptMap = {
      ja: `「${currentCity}」という地名・観光地名について、すべて日本語で以下のJSONのみ返してください。他のテキスト不要：{"name":"日本語での正式名称または一般的な呼び方（例：ロンドン、馬籠宿）","location":"都道府県・国など所在地（日本語で）","desc":"旅行者向け1〜2文の簡潔な説明（日本語で）"}`,
      en: `For the destination "${currentCity}", return ONLY this JSON in English, no other text: {"name":"official or common name in English","location":"region/country in English","desc":"1-2 sentence description for travelers in English"}`,
      zh: `关于地名"${currentCity}"，只用中文返回以下JSON，不要其他文字：{"name":"中文正式名称","location":"所在地区/国家（中文）","desc":"给旅行者的1-2句简介（中文）"}`,
      ko: `"${currentCity}" 지명에 대해 한국어로만 아래 JSON을 반환하세요. 다른 텍스트 불필요: {"name":"한국어 정식 명칭","location":"소재지/국가（한국어）","desc":"여행자를 위한 한국어 1-2문장 설명"}`,
    };
    ...
    nameEl.textContent = info.name + (info.location ? '　' + info.location : '');
    descEl.textContent = info.desc || '';
    document.getElementById('city-confirm-question').textContent = cl.confirm;
    confirmedCity = info.name || currentCity;
    confirmedLocation = info.location || '';
  } catch(e) {
...
let tripTiming = 'plan'; // 'plan' or 'now'
...
function renderResult(plan) {
  ...
  // Badges（設問の順番通り：スタイル・予算・食事・メンバー・日数）
